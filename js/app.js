...
$(function() {
  ...
  $(".mxd-nav__wrap").each(function() {
    ...
    // Trigger your hamburger menu toggle
    hamburgerEl.trigger("click");

    // Grab the target from data-scroll attribute
    const targetId = $(this).data("scroll");
    if (targetId) {
        // Use jQuery to find target element
        const $target = $("#" + targetId);
        if ($target.length) {
            // Animate smooth scroll with jQuery's animate
            $("html, body").animate({
                scrollTop: $target.offset().top
...
$('a[href^="#"]').on('click', function(e) {
    ...
    if ($target.length) {
        ...
        console.warn(`No element found for ${target}`);
    }
});
...
function getCurrentTheme(){
  let theme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'light' : 'light';
